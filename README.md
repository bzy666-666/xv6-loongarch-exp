...
cd qemu-9.2.0
# 基础配置
./configure --target-list=loongarch64-softmmu --prefix=/path/to/your/install/directory
# 安装
make -j$(nproc)
sudo make install
# 验证安装
...
如果在 `./configure` 阶段报告缺少库或工具，请根据错误提示安装对应的开发包。

3、下载交叉编译器，可使用以下命令进行安装
...
```bash
export PATH=/path/to/your/cross-tools/bin:$PATH
```
